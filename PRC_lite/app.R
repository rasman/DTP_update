# Monte Carlo simulation of Titration Paradox for PRC
# July, 2020
# St.Gallen, Switzerland
# Thomas Schnider, Prof. Dr. med.
# Charles Minto, MB ChB

# code update June 2024 to demonstrate PRC
#
# Elie Sarraf, M.D. C.M.
  ...
  res <- do.call(rbind, apply(
    as.matrix(D50_ind), 1, function(D50) {
      ...
      for (i in 2:nSteps) {
        ...
        error_calc <- ((targetEffect - effectToUse) > 0) * 2 -1
		
    		if (i == 2){
    			prior_calc <-error_calc
    		}
		
    		if (prior_calc*error_calc < -1) {
    		  statefound<-TRUE
    		  #firstdose <-newDose
    		}
            ...
            if (statefound){
              newDose <- newDose + firstdose * (error_calc*startVal$slow_facor)
            } else{
              newDose <- newDose + firstdose * (error_calc*startVal$fast_factor)
            }
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        sliderInput("doseScalarEffect",
          HTML(paste0(
            "Scalar ", "&#946;",
            tags$sub("1"), " (scales ", "Fast titration)"
          )),
          min = 0,
		  max = .5,
          step = .01,
        ...
        sliderInput("doseScalarDose",
          HTML(paste0(
            "Scalar ", "&#946;",
            tags$sub("2"), " (scales ", "Slow Titration)"
          )),
          min = 0,
          max = 0.2, 
          step = 0.001,
  ...
  server <- function(input, output) {
    ...
    input_Changed <- reactive({
      ...
      startVal <- list(
        ...
        startDose = input$startDose, slow_facor = input$doseScalarDose,
        fast_factor = input$doseScalarEffect, effectError = input$effectError, prior_error = 0
      )
