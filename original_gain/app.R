# Monte Carlo simulation of Titration Paradox
# July, 2020
# St.Gallen, Switzerland
# Thomas Schnider, Prof. Dr. med.
# Charles Minto, MB ChB

# Code update to increase the maximum value of the scaleD_Effect weight and
# showing the correlation (error) as function of the step
# May, 2024
# Elie Sarraf, M.D. C.M.
...
delta_dose <- function(measEff, currentDose, targetEff, hill1Deriv, startVal) {
  ...
  return(deviationFromMeanEffect_Dose * startVal$scaleD_Effect + deviationFromMeanDose_Dose * startVal$scaleD_Dose)
}
  ...
  server <- function(input, output) {
    ...
    input_Changed <- reactive({
      ...
      startVal <- list(
        ...
        startDose = input$startDose, scaleD_Dose = input$doseScalarDose,
        scaleD_Effect = input$doseScalarEffect, effectError = input$effectError
      )
